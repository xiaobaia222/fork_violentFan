...
unsigned char data TimeIndex = 0;		//换相时间保存索引
...
void Timer0Reload(unsigned int n)
{
	n = 65535 - n;
	TR0 = 0;
	TH0 = n >> 8;
...
unsigned int Timer1GetCnt(void)
{
	unsigned int temp;
	TR1 = 0;
	temp = TH1;
...
void CMP_Isr(void) interrupt 21	//比较器中断函数, 检测到反电动势过0事件
{
	...
	{
		PhaseTimeTmp[TimeIndex] = Timer1GetCnt();
		if(B_Timer1_OF)
		{
			B_Timer1_OF = 0;
			PhaseTimeTmp[TimeIndex] = 65535;
		}
		TimeIndex = (++TimeIndex) % 8;		//累加8次
		for(PhaseTime=0, i=0; i<8; i++)	//求8次换相时间累加和
			PhaseTime += PhaseTimeTmp[i];	
		PhaseTime = PhaseTime >> 3;			//求8次换相时间的平均值
		if((PhaseTime >= 40) && (PhaseTime <= 8192))
			motor_stuck_time = 0;
		/*	
		if(PhaseTime >= 80)
